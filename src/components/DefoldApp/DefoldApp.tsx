...
export function DefoldApp({ root, app, width, height, fullscreen = false }: DefoldAppProps) {
  const [ id ] = useState(() => `canvas-${UniqueCanvasId++}`);
  const canvas = useRef(null);
  const parent = useRef(null);
  const { loading, error, complete } = useEmbedScript(`${root}/dmloader.js`);
  console.log("DefoldApp", { root, app, width, height, fullscreen });

  useEffect(() => {
    if (!complete) return;

    console.log("DefoldApp");
